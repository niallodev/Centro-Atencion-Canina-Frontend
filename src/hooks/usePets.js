...
export const usePets = () => {
  ...
  if (mascotaActual?.categoriaId) {
    ...
    console.log(selectRazas.categoriaId);
    console.log(mascotaActual.categoriaId);
    const filtradas = selectRazas.filter(raza => raza.categoriaId === Number(mascotaActual.categoriaId));
    console.log(filtradas);
    setRazasFiltradas(filtradas);
  } else {
    console.log("FALSE");
    // Si no se ha seleccionado categoría, mostrar todas
    setRazasFiltradas(selectRazas);
  }
  ...
  return {
    ...
    handleSubmit, handleEliminarConfirmado, selectPet, selectMascotas,
  };
};
